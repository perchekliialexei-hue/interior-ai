...
import { ArrowLeft, ArrowRight, Check, Loader2, Building2, Send, Star, ChevronRight } from 'lucide-react';
...
export default function RealtorPage() {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', agency: '', address: '',
    goal: 'Продажа', roomType: 'Гостиная', style: 'Современный',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch('/api/order', {
        ...
        signal: ac.signal,
        body: JSON.stringify({
          ...rest, photoCount: formData.photos.length, isRealtor: true,
        ...
        <div className="grid md:grid-cols-4 gap-5">
          {[
            ...
            <div key={i} className="relative">
              {i < 3 && <div className="hidden md:block absolute top-6 left-[calc(100%-8px)] w-4 text-gray-700 text-lg z-10">→</div>}
                ...
                <motion.div key="progress"
                      ...
                      return (
                        <motion.div key={step.id} initial={{ opacity: 0, x: -10 }}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className={`text-sm font-medium ${isActive ? 'text-white' : isDone ? 'text-gray-400' : 'text-gray-600'}`}>{step.label}</div>
                            {isActive && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xs text-violet-400 mt-0.5">{step.sublabel}</motion.div>}
                          </div>
                  ...
                  <div className="mt-4 h-1 bg-white/5 rounded-full overflow-hidden">
                    <motion.div className="h-full bg-violet-600 rounded-full"
                      animate={{ width: `${(currentStep / STEPS.length) * 100}%` }} transition={{ duration: 0.5 }} />
                  </div>
                ...
                <motion.form key="form" onSubmit={handleSubmit} className="space-y-4" initial={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="text" required value={formData.name} onChange={e => set('name', e.target.value)} className={inputCls} placeholder="Алексей" />
                    </div>
                    <div>
                      <label className="text-sm text-gray-400 block mb-1">Агентство</label>
                      <input type="text" value={formData.agency} onChange={e => set('agency', e.target.value)} className={inputCls} placeholder="NeoImobil" />
                    </div>
                  </div>

                  <div>
                    <label className="text-sm text-gray-400 block mb-1">Email</label>
                    <input type="email" required value={formData.email} onChange={e => set('email', e.target.value)} className={inputCls} placeholder="alex@agency.md" />
                  </div>

                  <div>
                    <label className="text-sm text-gray-400 block mb-1">Адрес объекта</label>
                    <input type="text" value={formData.address} onChange={e => set('address', e.target.value)} className={inputCls} placeholder="ул. Штефан чел Маре, 12, Кишинёв" />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea placeholder="Например: светлые тона, нейтральный стиль..."
                  ...
                  <p className="text-center text-xs text-gray-600">Первый объект бесплатно · Результат за ~2 минуты</p>
                </motion.form>
              )}
            </AnimatePresence>
