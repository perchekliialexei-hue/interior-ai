...
// FIX 1: placedBoxes теперь создаётся в useEffect, не на уровне модуля
// (передаём как аргумент во все функции)
...
function resolveCollision(
  ...
  // FIX 2: стена ограничивает перпендикулярное смещение, не параллельное
  // back/front стена → мебель вдоль X → коллизии только по X (не по Z)
  const constrainedAxis =
    wall === 'back' || wall === 'front' ? 'x' :
    wall === 'left' || wall === 'right' ? 'z' :
    null;

  for (let iter = 0; iter < 8; iter++) {
    let moved = false;
    for (const b of placedBoxes) {
      if (type === 'rug' || b.type === 'rug') continue;
      ...
      if (overlapX > 0 && overlapZ > 0) {
        const pushX = px >= b.x ? overlapX + 0.06 : -(overlapX + 0.06);
        const pushZ = pz >= b.z ? overlapZ + 0.06 : -(overlapZ + 0.06);

        if (constrainedAxis === 'x') {
          px += pushX;
        } else if (constrainedAxis === 'z') {
          pz += pushZ;
...
// FIX 3: snapToWall принимает оригинальные (не ротированные) размеры iw/id
// и сам вычисляет правильный отступ в зависимости от стены
function snapToWall(
  px: number, pz: number,
  iw: number, id: number,
  rot90: boolean,
  ...
  const G = 0.015;
  // Размер вдоль оси X и Z в мировых координатах
  const extX = rot90 ? id : iw;  // сколько занимает по X
  const extZ = rot90 ? iw : id;  // сколько занимает по Z
  switch (wall) {
    case 'back':  return [Math.max(extX/2+G, Math.min(W-extX/2-G, px)), extZ/2+G];
    case 'front': return [Math.max(extX/2+G, Math.min(W-extX/2-G, px)), L-extZ/2-G];
    case 'left':  return [extX/2+G, Math.max(extZ/2+G, Math.min(L-extZ/2-G, pz))];
    case 'right': return [W-extX/2-G, Math.max(extZ/2+G, Math.min(L-extZ/2-G, pz))];
...
function addCurtains(g: THREE.Group, w: number, h: number, c: number) {
  const curtainColor = Math.max(c, 0x9A9080);
...
function addPainting(g: THREE.Group, w: number, h: number, c: number) {
  const frameMat = mat(0x2A2520, 0.5, 0.15);
  const ft = 0.04;
...
function addBed(g: THREE.Group, w: number, d: number, c: number) {
  ...
  // Изголовье — мягкое (ткань поверх дерева)
  add(g, box(w,0.55,0.08), wood, 0, 0.44, -d/2+0.04);
  add(g, box(w-0.08,0.40,0.04), mat(fabC,0.92), 0, 0.44, -d/2+0.085);
  // Мягкая панель изголовья (чуть светлее ткань)
  add(g, box(w-0.16,0.32,0.03), mat(Math.min(0xFFFFFF, fabC+0x101010),0.96), 0, 0.47, -d/2+0.115);
}
...
// ── CONTACT SHADOWS ───────────────────────────────────────────────────────────
// FIX 4: MultiplyBlending вместо NormalBlending — тени не будут белыми/серыми
function addContactShadow(scene: THREE.Scene, px: number, pz: number, sw: number, sd: number) {
  ...
  inner.position.set(px, 0.003, pz);
  inner.renderOrder = 1;
  scene.add(inner);
  ...
  outer.position.set(px, 0.002, pz);
  outer.renderOrder = 0;
  scene.add(outer);
}
...
function itemPriority(item: any): number {
  ...
  return FURNITURE_PRIORITY[type] ?? 3;
}

// FIX 5: guessWall принимает уже клиппированные координаты
...
function wallToRotation(wall: string): number {
  // back (z=0): front +Z смотрит в комнату → 0
  // front (z=L): нужно -Z → 180
  // left (x=0): нужно +X → 270° (Three.js CCW: 270° = -90°, поворачивает +Z → +X ✓)
  // right (x=W): нужно -X → 90° (поворачивает +Z → -X ✓)
  return wall === 'back' ? 0 : wall === 'front' ? 180 : wall === 'left' ? 270 : 90;
}

// ── MASTER PLACER ─────────────────────────────────────────────────────────────
function place(scene: THREE.Scene, item: any, W: number, L: number, H: number, placedBoxes: BBox[]) {
  const type = TYPE_MAP[item.type] ?? item.type ?? 'table';
  const fi = { ...item, type };
  ...
  if (fi.type === 'painting') {
    ...
    const px = typeof fi.x === 'number' ? fi.x : W/2;
    const pz = typeof fi.z === 'number' ? fi.z : L/2;
    // FIX 5: клиппируем координаты перед guessWall
    const cpx = Math.max(0.1, Math.min(W-0.1, px));
    const cpz = Math.max(0.1, Math.min(L-0.1, pz));
    const wall = fi.wall || guessWall(cpx, cpz, W, L);
    const hangY = H * 0.62;
    if (wall === 'back') {
      g.position.set(Math.max(iw/2+0.1, Math.min(W-iw/2-0.1, cpx)), hangY, 0.04);
    } else if (wall === 'front') {
      g.position.set(Math.max(iw/2+0.1, Math.min(W-iw/2-0.1, cpx)), hangY, L-0.04);
      g.rotation.y = Math.PI;
    } else if (wall === 'left') {
      g.position.set(0.04, hangY, Math.max(iw/2+0.1, Math.min(L-iw/2-0.1, cpz)));
      g.rotation.y = Math.PI/2;
    } else {
      g.position.set(W-0.04, hangY, Math.max(iw/2+0.1, Math.min(L-iw/2-0.1, cpz)));
      g.rotation.y = -Math.PI/2;
    }
    scene.add(g);
  ...
  // FIX 5: клиппируем сначала, потом guessWall
  let px = typeof fi.x === 'number' && !isNaN(fi.x) ? fi.x : W/2;
  let pz = typeof fi.z === 'number' && !isNaN(fi.z) ? fi.z : L/2;
  px = Math.max(0.1, Math.min(W-0.1, px));
  pz = Math.max(0.1, Math.min(L-0.1, pz));

  let rotDeg: number;
  if (isWallType) {
    if (!wall || wall === 'none') wall = guessWall(px, pz, W, L);
  ...
  // FIX 3: snap/collision используют мировые размеры (с учётом поворота)
  const worldW = rot90 ? id : iw;  // размер вдоль X
  const worldD = rot90 ? iw : id;  // размер вдоль Z

  // Клиппинг с учётом реального размера
  px = Math.max(worldW/2+0.02, Math.min(W-worldW/2-0.02, px));
  pz = Math.max(worldD/2+0.02, Math.min(L-worldD/2-0.02, pz));

  // Snap to wall (передаём оригинальные iw/id и rot90)
  if (wall) [px, pz] = snapToWall(px, pz, iw, id, rot90, W, L, wall);

  // Collision detection (мировые размеры)
  [px, pz] = resolveCollision(px, pz, worldW, worldD, fi.type, W, L, wall, placedBoxes);

  // Финальный snap после коллизий
  if (wall) [px, pz] = snapToWall(px, pz, iw, id, rot90, W, L, wall);

  if (fi.type !== 'rug') {
    placedBoxes.push({ x: px, z: pz, w: worldW, d: worldD, type: fi.type });
  }

  if (fi.type !== 'rug') addContactShadow(scene, px, pz, worldW, worldD);
  ...
  // Размеры для геометрии — в локальной системе объекта (до поворота)
  const lw = iw;  // local width (вдоль локального X = до поворота)
  const ld = id;  // local depth (вдоль локального Z = до поворота)

  switch (fi.type) {
    case 'bed':        addBed(g, lw, ld, c);                          break;
    case 'sofa':       addSofa(g, lw, ld, c);                         break;
    case 'wardrobe':   addWardrobe(g, lw, ld, Math.max(1.6, ih), c);  break;
    case 'dresser':    addDresser(g, lw, ld, Math.max(0.55, ih), c);  break;
    case 'desk':       addDesk(g, lw, ld, c);                         break;
    case 'chair':      addChair(g, c);                                 break;
    case 'table':      addTable(g, lw, ld, c);                        break;
    case 'shelf':      addShelf(g, lw, ld, Math.max(1.4, ih), c);     break;
    case 'lamp':       addLamp(g, c);                                  break;
    case 'plant':      addPlant(g, c);                                 break;
    case 'rug':        addRug(g, lw, ld, c);                          break;
    case 'nightstand': addNightstand(g, lw, ld, Math.max(0.45, ih), c); break;
    default:           add(g, box(lw, Math.max(0.3,ih), ld), mat(c), 0, Math.max(0.3,ih)/2, 0);
  }
}
...
function ViewerContent() {
  ...
  useEffect(() => {
    ...
    // FIX 1: placedBoxes создаётся локально в useEffect — не течёт между рендерами
    ...
    // FIX 6: снижаем exposure — ACES при 1.18 перегревает цвета (особенно дерево)
    ...
    const sh = 0.065, sdd = 0.022;
    [[W, sh, sdd, W/2, sh/2, sdd/2], [W, sh, sdd, W/2, sh/2, L-sdd/2],
     [sdd, sh, L, sdd/2, sh/2, L/2], [sdd, sh, L, W-sdd/2, sh/2, L/2]].forEach(([bW,bH,bD,bx,by,bz]) => {
    ...
    [[winW+ft*2, ft, wx, winY+winH/2+ft/2, 0.05],
     [winW+ft*2, ft, wx, winY-winH/2-ft/2, 0.05]].forEach(([fw,,bx,by,bz]) => {
    ...
    // Массивы для синхронной прозрачности стен
    ...
    if (d?.furniture?.length > 0) {
      [...d.furniture]
        .sort((a: any, b: any) => itemPriority(b) - itemPriority(a))
        .forEach((item: any) => place(scene, item, W, L, H, placedBoxes));

      scene.children.forEach(obj => {
        if (!(obj instanceof THREE.Group)) return;
        const pos = obj.position;
        if (pos.z < 0.3)       backWallObjects.push(obj);
        else if (pos.x < 0.3)  leftWallObjects.push(obj);
        else if (pos.x > W-0.3) rightWallObjects.push(obj);
        else if (pos.z > L-0.3) frontWallObjects.push(obj);
      });
    } else {
      // Демо-комната
      place(scene,{type:'bed',       x:W*0.5, z:L*0.18,width:1.6,depth:2.0,height:0.5, color:'#8B7355',wall:'back'},W,L,H,placedBoxes);
      place(scene,{type:'nightstand',x:W*0.20,z:L*0.18,width:0.5,depth:0.4,height:0.55,color:'#7A6345',wall:'back'},W,L,H,placedBoxes);
      place(scene,{type:'wardrobe',  x:W*0.12,z:L*0.55,width:1.0,depth:0.5,height:2.0, color:'#9A8365',wall:'left'},W,L,H,placedBoxes);
      place(scene,{type:'lamp',      x:W*0.82,z:L*0.72,width:0.3,depth:0.3,height:1.5, color:'#A09070'},W,L,H,placedBoxes);
      place(scene,{type:'plant',     x:W*0.88,z:L*0.88,width:0.3,depth:0.3,height:0.55,color:'#4A7A5A'},W,L,H,placedBoxes);
      place(scene,{type:'curtains',  x:W/2,   z:0,     width:1.8,depth:0.1,height:2.4, color:'#E8E0D5',wall:'back'},W,L,H,placedBoxes);
      place(scene,{type:'painting',  x:W*0.7, z:0,     width:0.8,depth:0.05,height:0.6,color:'#C4B89A',wall:'back'},W,L,H,placedBoxes);
      place(scene,{type:'rug',       x:W*0.5, z:L*0.55,width:1.6,depth:2.0,height:0.02,color:'#B8A882'},W,L,H,placedBoxes);
    }
