  ...
  function snapToWall(px: number, pz: number, sw: number, sd: number, W: number, L: number, wall: string): [number, number] {
    ...
    switch (wall) {
      ...
      case 'left':  return [sw/2+G,   Math.max(sd/2+G, Math.min(L-sd/2-G, pz))];
      case 'right': return [W-sw/2-G, Math.max(sd/2+G, Math.min(L-sd/2-G, pz))];
  ...
  function wallToRotation(wall: string): number {
    return wall==='back'?0 : wall==='front'?180 : wall==='left'?90 : 270;
  }
...
// guessWall до rot90 — используем сырые координаты, но сначала клиппим в границы комнаты
px = Math.max(0.02, Math.min(W-0.02, px));
pz = Math.max(0.02, Math.min(L-0.02, pz));
...
pz = Math.max(snapD/2+0.02, Math.min(L-snapD/2-0.02, pz));
    ...
    // Collision detection
    [px, pz] = resolveCollision(px, pz, snapW, snapD, fi.type, W, L, wall);
    ...
    useEffect(() => {
      ...
      if (d?.furniture?.length > 0) {
        [...d.furniture]
          ...
          .forEach((item: any) => place(scene, item, W, L, H));
